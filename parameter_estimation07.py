...
try:
    ...
    filename = ''
    SLURM_ID = np.random.randint(0,10) 
...
# variable_X = np.array([1.2, 4],dtype=np.float64)

algorithm_param = {'max_num_iteration': ga_iteration,\
                    'population_size':500,\
...
# params = []
# sses_ga=[]    
model=ga(function=error_func_idm,\
...
ga_obj = model.output_dict['function']

com_time = datetime.now() - start

results.append(np.append(param,[alpha,ga_obj,com_time,nth_car]))


# import timeit 

# %timeit error_func_idm(variable_X)


#### Fractional 

for alpha in np.linspace(0.01,.9999,10): 

    start = datetime.now()

    varbound=np.array([[a*lf,a*uf],[lf*delta,uf*delta],[lf*beta,uf*beta],[lf*alpha,uf*alpha]])
    ...
    # params = []
    # sses_ga=[]    
    model=ga(function=error_func_fidm,\
                dimension=len(varbound),\
                variable_type='real',\
                variable_boundaries=varbound,\
                algorithm_parameters=algorithm_param)
    ...
    ga_obj = model.output_dict['function']
    com_time = datetime.now() - start
    results.append(np.append(param,[ga_obj,com_time,nth_car]))
    
df = pd.DataFrame(results,columns=['a','delta','beta','alpha','objective','time','car'])

# df.to_csv('pe_results_version_test.csv') 

# df.to_csv('pe_results_version%s_car%i.csv' %(version,nth_car)) 
      
# df.to_csv(output_folder+'pe_results_version%s_car%i.csv' %(version,nth_car))

df.to_csv(output_folder+'pe_results_version%s_car%i.csv' %(version,nth_car), index = None)    
